...
import { generateSecureId, getCurrentDateFormatted } from "@/utils/logics";
import {
  collection,
  deleteDoc,
  doc,
...
export const UploadBookingProgress1 = async (moveObj) => {
  ...
  const { serviceLocation, personalDetails, moveDetails, stage, activity } =
    moveObj;
  ...
  const data = {
    ...
    // moveDateRaw,
    stage,
    activity: [{ name: activity, date: getCurrentDateFormatted() }],
    bookingId,
    quoteRef,
    quoteType: "online",
