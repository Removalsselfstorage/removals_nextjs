...
import { checkBookStatus } from "@/utils/logics";
...
const JobBoard = ({  }) => {
  ...
  useEffect(() => {
    ...
    const currentSmartMb = smartMb?.filter((ad) => {
        ...
        <section className='mb-[40px] w-full px-[10px] md:px-[30px] '>
          ...
          <div className='rounded-[10px] w-full bg-secondary/5 flex flex-col space-y-[20px] md:space-y-0 md:flex-row md:justify-between  md:items-center py-[30px] px-[20px] text-[15px]'>
            <div className='flex space-x-[20px] items-center'>
              <IoReceipt className='text-secondary text-[40px] md:text-[60px]' />
              <div className='flex flex-col md:text-[18px]'>
                <p className=''>Available to pick up a new move?</p>
                <p className='mt-[0px] w-full font-bold'>
                  Just Apply for the move!
                </p>
              </div>
            </div>
            <div
              className='btn btn-outline btn-wide btn-secondary'
              onClick={() => window.my_modal_399.showModal()}
            >
              <span>Learn about applying</span>
              <FaAngleRight />
            </div>
          </div>
          {/* * modal */}
          <dialog id='my_modal_399' className='modal py-[20px] px-[10px]'>
            <form method='dialog' className='modal-box '>
              ...
              <div className='w-full flex-col items-center'>
                <h3 className='font-bold text-[22px] text-secondary px-[20px] text-center'>
                  Keep these things in mind, when applying for new moves
                </h3>
              </div>
